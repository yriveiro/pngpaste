...
download() {
	...
	if has curl; then
		curl -# -L -o "$file" "$url" && return 0 || rc=$?
		cmd="curl -# -L -o \"$file\" \"$url\""
	elif has wget; then
		wget --quiet --output-document="$file" "$url" && return 0 || rc=$?
		cmd="wget --quiet --output-document=\"$file\" \"$url\""
	elif has fetch; then
		fetch --quiet --output="$file" "$url" && return 0 || rc=$?
	...
	error "Command failed (exit code $rc): ${BLUE}${cmd}${NC}"
	echo "" >&2
	info "This is likely due to ${DISPLAY_NAME} not yet supporting your configuration."
...
install() {
	...
	if test_writeable "${BIN_DIR}"; then
		sudo=""
		msg="Installing ${DISPLAY_NAME}, please wait..."
	else
		warn "Escalated permissions are required to install to ${BIN_DIR}"
		elevate_priv
		sudo="sudo"
		msg="Installing ${DISPLAY_NAME} as root, please wait..."
	fi
...
confirm() {
	if [[ -z "${FORCE-}" ]]; then
		printf "%s " "${MAGENTA}?${NC} $* ${BOLD}[y/N]${NC}"
		set +e
		read -r yn </dev/tty
		rc=$?
		set -e
